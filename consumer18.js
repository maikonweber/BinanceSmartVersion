     ...
     objetoRolleta.detectStrategy.colunasRepeat.forEach(async (coluna) => {
          ...
          objetoRolleta.detectStrategy.colorRepeat.forEach(
               async (color) => {
                    ...
                    await regExe(color.color, objetoRolleta)
               }
          )

          objetoRolleta.detectStrategy.arrayBloco1Ausencia.forEach(
               async (colunas) => {
                    await regExe(colunas.coluna, objetoRolleta)
               }
          )
...
setInterval ( async () => {
            ...
            LastNames.rows.forEach(async element => {
               ...
               return strategyProced(objectAnalyser)       
               })
